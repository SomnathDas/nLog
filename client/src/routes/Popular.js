...
const Popular = () => {
    ...
    try {
      if (page <= lastPage) {
        const response = await axios.get(
          `/api/posts/popular?limit=4&page=${page}`
        );
        setPage(page + 1);
  ...
  return (
    <section
      className={`h-screen w-screen flex flex-col-reverse lg:flex-row lg:items-start `}
    >
              ...
              return (
                <div
                  className={`${index === 0 ? "border-2 border-primary" : ""} ${
                    index === 1 ? "border-2 border-lime-300" : ""
                  } ${index === 2 ? "border-2 border-pink-300" : ""} ${
                    index > 2 ? "border-2 border-gray-700" : ""
                  } p-12`}
                  key={elem._id + `${index}`}
                >
                  <Feed
